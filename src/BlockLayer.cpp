...
const char* BlockLayer::SIZE           = "size";
const char* BlockLayer::ACTIVATION     = "activation";
const char* BlockLayer::INITIALIZATION = "init";
const char* BlockLayer::USEBIAS        = "useBias";
const char* BlockLayer::DEBUG          = "debug";
const char* BlockLayer::PRUNING        = "pruning";
const char* BlockLayer::POSWEIGHTS     = "posWeights";
...

{
  ...
  {
    Ref<BlockLayer> image = dynamicCast<BlockLayer> ( layer );
    ...
    iInpWts_.resize ( nBlocks_, 6 );
    iInpWts_ = -1;

    weightsDof_.resize ( nBlocks_, 6);
    weightsDofcopy_.resize ( nBlocks_, 6);
 
    image->getInpDofs ( iInpWts_ );

    System::out() << "dofs copied " << iInpWts_ << "\n";

    weights_.resize ( nBlocks_*blockDim_, blockDim_ );
    weights_ = 0.0;

    weightsExp_.resize ( nBlocks_*blockSize_, blockSize_ );
    weightsExp_ = 0.0;
  ...
  else if ( globdat.find ( inpNeurons_, LearningParams::PREDECESSOR ) )
  {
    ...
    IdxVector newaxons  = aset->addAxons ( 6 * nBlocks_ );
    System::out() << "Block layer. inpWeights: Creating " << 6 * nBlocks_ << " axons\n";
...

{
  ...
  if ( debug_ ) System::out() << "BlockLayer. update. weights_ " << weightsDof_ << "\n";
  //if ( debug_ ) System::out() << "BlockLayer. update. weightsExp_ " << weightsExp_ << "\n";
...

{
  ...
  for ( idx_t p = 0; p < data->batchSize(); p++ )
  {
	...
	defgradblock(2,2) += 1.0;
        ...
        if ( debug_ ) System::out() << "defgrad x weights (I + grad u) = " << defgradblock << "\n";

        // Applying softplus on the diagonal elements
        // of F to avoid interpenetration
        
//        Matrix defgradblockact = defgradblock.clone();

   //     actfuncweights_ ( defgradblockact ); 

        if ( debug_ ) System::out() << "defgrad with sp on diagonal: " << defgradblock << "\n";
	...
	if ( defgradblock(0,0) <= 0.0 || defgradblock(1,1) <= 0.0 || defgradblock(2,2) <= 0.0 )
	{
	  negdiag = true;
	}

	if ( detF <= 0.0 || negdiag == true  )
	{
	   System::out() << "Negative det! Setting to undeformed configuration.\n";
           System::out() << "Block " << nb << " batch " << p << " weights: \n";
           System::out() << actWeights << "\n";
           System::out() << "defgrad " << defgradblock << "\n";
	   Matrix ideye ( blockDim_, blockDim_ );
	   ideye = 0.0;
	   ideye(0,0) = ideye(1,1) = ideye(2,2) = 1.0;
	   for ( idx_t i = 0; i < blockDim_; i++ )
           {
            for ( idx_t j = 0; j < blockDim_; j++ )
            {
              actValues( nb*blockSize_ + i*blockDim_ +j, p) = ideye(i,j);
            }
           }
	}
	else
...
//	System::out() << "Checkpoint " << nb << " detWact " << determinant ( actWeights ) << " w " << actWeights << " detF " << detF << "\n";
  ...
  select ( data->activations, iNeurons_, ALL ) = actValues;
...
void BlockLayer::backPropagate_
...

{
  ...
  if ( debug_ ) System::out() << "backPropagate. BlockLayer. derivs (values): " << derivs << "\n";
  ...
  for ( idx_t p = 0; p < data->batchSize(); p++ )
  {
     for ( idx_t nb = 0; nb < nBlocks_; nb++ )
     {
	   ...
	   if ( detF <= 0.0 || negdiag == true)
	   {
	     System::out() << "Negative determinant! Setting zero contribution.\n";
	     defgradblock = 0.0;
	   }
	   else
	   {
  ...
  if ( debug_ ) System::out() << "backPropagate. BlockLayer. derivs (grad_): " << derivs << "\n";
  ...
  if ( debug_ ) System::out() << "backPropagate. BlockLayer. acts: " << acts << "\n";
  ...
  if ( debug_ ) 
  {
    System::out() << "backPropagate. BlockLayer. iNeuros: " << iNeurons_ << ".\n";
    System::out() << "backPropagate. BlockLayer. delta: " << select ( data->deltas, iNeurons_, ALL ) << ".\n";
    System::out() << "backPropagate. BlockLayer. delta*derivs: " << deltas << "\n";
    System::out() << "backPropagate. BlockLayer. derivs: " << derivs << "\n";
    System::out() << "backPropagate. BlockLayer. acts " << acts.size(0) << " x " << acts.size(1) << "\n";
  }

  Matrix gradmat ( nBlocks_, 6 );
  gradmat = 0.0;
  bpalloc_.stop();
  bpmmul_.start();
...
//  System::out() << "acts all " << acts << "\n";
  ...
  if ( debug_ )
  {
    System::out() << "backPropagate. BlockLayer. deltas*acts: " << gradmat << "\n";
 //   System::out() << "backPropagate. BlockLayer. size_: " << size_ << "\n";
 //   System::out() << "backPropagate. BlockLayer. iInptWts: " << iInpWts_ << "\n";
 //   System::out() << "backPropagate. BlockLayer. gradmat dim " << gradmat.size(0) << " x " << gradmat.size(1) << "\n";
  }
  ...
  for ( idx_t in = 0; in < nBlocks_; ++in )
  {
    ...
    if ( useBias_ )
    {
      ...
      if ( debug_ ) System::out() << "backPropagate. BlockLayer. biases " << iBiases_[in] << ": " << sum ( deltas(in,ALL) ) << "\n";
    }
    bpbias_.stop();
  }
 ...
 /* if ( debug_ ) 
  {
    System::out() << "backPropagate. BlockLayer. inpNeurons: " << inpNeurons_ << "\n";
    System::out() << "backPropagate. BlockLayer. deltas (inp): " << select (data->deltas, inpNeurons_, ALL) << "\n";
...
void BlockLayer::getDeterminant_
...
void BlockLayer::forwardJacobian_
...

{
  ...
  if ( debug_ ) 
  {
     System::out() << "BlockLayer. getJacobian initial:\n" << jacobian_ << "\n";
  ...
  if ( debug_ )
  {
    System::out() << "BlockLayer. grad_ ( derivs ):\n" << derivs << "\n";
    System::out() << "weights_:\n" << weights_ << "\n";
  }
  ...
  if ( debug_ ) System::out() << "BlockLayer. getJacobian updated:\n" << jacobian_ << "\n";
...

{
  ...
  return newInstance<BlockLayer> ( name, conf, props, globdat );
}
...
void declareBlockLayer ()
{
  using jive::model::ModelFactory;

  // Register the StressModel with the ModelFactory.

  ModelFactory::declare ( "Block", newBlockLayer );
}
