...
const char* ModDenseLayer::SIZE           = "size";
const char* ModDenseLayer::ACTIVATION     = "activation";
const char* ModDenseLayer::INITIALIZATION = "init";
const char* ModDenseLayer::USEBIAS        = "useBias";
const char* ModDenseLayer::DEBUG          = "debug";
const char* ModDenseLayer::PRUNING        = "pruning";

//-----------------------------------------------------------------------
//   constructor & destructor
//-----------------------------------------------------------------------

ModDenseLayer::ModDenseLayer
...

{
  ...
  {
    Ref<ModDenseLayer> image = dynamicCast<ModDenseLayer> ( layer );

    System::out() << "Mod dense layer. Symmetric network.\n";
  ...
  {
    IdxVector idofs ( nBlocksPrev_*nWtsBlock_ );      // Number of dofs per component 
    
    iInpWts_.resize ( nBlocks_*nWtsBlock_, nBlocksPrev_*nWtsBlock_ );  // Dofs of weights
    iInpWts_ = -1;
    ...
    IdxVector newaxons  = aset->addAxons ( nWtsBlock_*nBlocksPrev_*nWtsBlock_ );
    System::out() << "Mod dense layer. inpWeights: Creating " << nWtsBlock_*nBlocksPrev_*nWtsBlock_ << " axons\n";
  ...
  //  System::out() << "newaxons: " << newaxons << "\n";
    ...
    for ( idx_t nb = 0; nb < nBlocks_; nb++ )
    {
      ...
      IdxVector selComp ( preSize_ - 3*nBlocksPrev_ );
      IdxVector remComp ( 3*nBlocksPrev_ );

      for ( idx_t in = 0; in < nComp_; in++ )
      {
        IdxVector myaxons ( newaxons[slice (nb*(nWtsBlock_*nBlocksPrev_)+symIndex_[in]*nBlocksPrev_*nWtsBlock_,nb*(nWtsBlock_*nBlocksPrev_)+(symIndex_[in]+1)*nBlocksPrev_*nWtsBlock_ )] );
        System::out() << "i: " << in << "\n";
        ...
        for ( idx_t ii = 0; ii < nBlocksPrev_; ii++) 
	{
	  ...
	  idx_t computedRemComp = 0;

	  for ( idx_t jj = 0; jj < 9; jj++ )
	  {
	    if ( jj == 3 || jj == 6 || jj == 7 ) 
	    { 
	      remComp[ii*3+computedRemComp] = ii*nComp_+jj;
	      computedRemComp += 1;
	    }
	    else
	    {
	      selComp[ii*6+computedSelComp] = ii*nComp_+jj;
	      computedSelComp += 1;
	    }
	  }
	}
...

{
  ...
  else
  {
    ...
    {
      weights_(in,ALL) = state[iInpWts_(in,ALL)];
     //System::out() << "Updated weights (Mod dense Layer): " << weights_(in, ALL) << "\n";
    }
  }
}
      ...
      for ( idx_t in = 0; in < nComp_; in++ )
      {
        for ( idx_t ii = 0; ii < nBlocksPrev_; ii++)
        {
          ...
          for ( idx_t jj = 0; jj < 9; jj++ )
          {
            ...
            actWeights(nb*nComp_+in, remComp) = 0.0;
        }
      }  
     }
...
void ModDenseLayer::getDeterminant_
...
void ModDenseLayer::backPropagate_
...

{
  ...
  if ( debug_ ) System::out() << "backPropagate. ModDenseLayer. derivs (values): " << derivs << "\n";

  bpfunc_.start();
  grad_ ( derivs );

  if ( debug_ ) System::out() << "backPropagate. ModDenseLayer. derivs (grad_): " << derivs << "\n";
  ...
  if ( debug_ ) System::out() << "backPropagate. ModDenseLayer. acts: " << acts << "\n";
  ...
  if ( debug_ ) 
  {
    System::out() << "backPropagate. ModDenseLayer. iNeuros: " << iNeurons_ << ".\n";
    System::out() << "backPropagate. ModDenseLayer. delta: " << select ( data->deltas, iNeurons_, ALL ) << ".\n";
    System::out() << "backPropagate. ModDenseLayer. delta*derivs: " << deltas << "\n";
  }
  ...
  if ( pruning_ )
  {
      ...
      for ( idx_t p = 0; p < nBlocksPrev_; p++ ) // Number of int. points in mat rec layer
      {
       for ( idx_t i = 0; i < nComp_; i++ ) // Number of components
       {
          if ( i == 3 || i == 6 || i == 7 )
          {
            weightscopy_(ALL, p*nComp_+i) = 0.0;
            actweightscopy_(ALL, p*nComp_+i) = 0.0;
          }
       }
      }
        
      if ( debug_ ) System::out() << "Weights after pruning " << weightscopy_ << "\n";
   }
  ...
  if ( debug_ )
  {
    System::out() << "backPropagate. ModDenseLayer. deltas*acts: " << gradmat << "\n";
    System::out() << "backPropagate. ModDenseLayer. size_: " << size_ << "\n";
    System::out() << "backPropagate. ModDenseLayer. iInpWts: " << iInpWts_ << "\n";
  }
...
//	System::out() << "comp " << in << gradmat(nb*nComp_+in, selComp) << "\n";
        if ( symmetric_ )
	{
	  if ( in != 3 && in != 6 && in != 7 ) 
  	  {
  ...
  else
  {
    for ( idx_t in = 0; in < size_; ++in )
    {
      ...
      if ( useBias_ )
      {
        ...
        if ( debug_ ) System::out() << "backPropagate. ModDenseLayer. biases " << iBiases_[in] << ": " << sum ( deltas(in,ALL) ) << "\n";
      }
      bpbias_.stop();
      
 //   System::out() << "backPropagate. grads Mod dense layer " << in << ": " << grads[iInpWts_(in, ALL)] << "\n";
    }
  } 
  ...
  if ( debug_ ) 
  {
    System::out() << "backPropagate. ModDenseLayer. inpNeurons: " << inpNeurons_ << "\n";
    System::out() << "backPropagate. ModDenseLayer. deltas (inp): " << select (data->deltas, inpNeurons_, ALL) << "\n";
...
void ModDenseLayer::forwardJacobian_
...
void ModDenseLayer::backJacobian_
...
void ModDenseLayer::getJacobian_
...

{
  ...
  if ( debug_ ) 
  {
     System::out() << "ModDenseLayer. getJacobian initial:\n" << jacobian_ << "\n";
...
*/
  ...
  if ( debug_ ) System::out() << "ModDenseLayer. getJacobian updated:\n" << jacobian_ << "\n";
...

{
  ...
  return newInstance<ModDenseLayer> ( name, conf, props, globdat );
}
...
void declareModDenseLayer ()
{
  using jive::model::ModelFactory;

  // Register the StressModel with the ModelFactory.

  ModelFactory::declare ( "ModDense", newModDenseLayer );
}
